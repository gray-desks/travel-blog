    ...
    <article className={styles.card}>
      ...
      <div className={styles.media}>
        ...
        <Link prefetch href={href} aria-label={article.title || '記事を見る'}>
          {img ? (
            ...
            <div className={styles.imgWrapper} aria-hidden="true" style={{ background: '#eee' }} />
          )}
        </Link>
        {/* 記事分類のラベル（CSSクラスは英語値を使用し、表示は日本語に変換） */}
        {article.type && (
          <span className={`${styles.label} ${styles[`label--${article.type}`]}`}>
      ...
      <div className={styles.body}>
          ...
          <div className={styles.gallery} aria-label="ギャラリー">
            {thumbs.map((url, idx) => (
              <Link key={idx} href={href} aria-label={`ギャラリー画像 ${idx + 1}`}>
                <ImageWithSpinner src={url} alt="" width={36} height={36} className={styles.thumbImg} />
              </Link>
            ))}
            {moreCount > 0 && (
              <Link href={href} className={styles.thumbMore} aria-label={`他 ${moreCount} 枚`}>
                +{moreCount}
              </Link>
            )}
          </div>
        ...
        <div className={styles.meta}>
          {article.prefecture && <span className={styles.metaItem} aria-label="都道府県">📍 {article.prefecture}</span>}
          {article.publishedAt && (
            // 時間情報は <time> 要素でマークアップし、ja-JP ロケールで日付表示
            <time dateTime={article.publishedAt} className={styles.metaItem} aria-label="公開日">
